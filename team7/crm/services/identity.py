...
@transaction.atomic
def ensure_user(user_id: str, username: Optional[str] = None):
    if not user_id:
        raise BadRequest("user_id is required")

    user, created = CmrUser.objects.get_or_create(
        id_user=user_id,
        defaults={"username": username or "Guest"}
    )

    # 🔴 IMPORTANT: do NOT overwrite username if user already exists
    # Only update username if it is empty/null
    if not created and username and not user.username:
