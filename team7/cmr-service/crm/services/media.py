...
def list_media(target_id: Optional[str] = None, status: str = "approved") -> List[Dict[str, Any]]:
    """
    - If target_id is provided: list media for that target.
    - If target_id is None: list media across ALL targets (needed for moderator dashboard).
    - status can be pending/approved/rejected/all.
    """
...
def upload_media(
    ...
    if not media_type:
        if ct.startswith("image/"):
            ...
            media_type = "image"  # default for demo
    ...
    # public_url should start with MEDIA_URL
    ...
    media = CmrMedia.objects.create(
        ...
        status="pending",   # ✅ pending until moderator approves
        report_count=0,
        created_at=now,
        deleted_at=None,
    )
...
def update_media_status(moderator_user_id: str, media_id: str, status: str) -> CmrMedia:
    ...
    # ✅ optional: when rejected, hide it completely
    if status == "rejected":
